...
public interface route_info_Dao {
    ...
    @Query("SELECT * from (SELECT station.name as src,  station.name as dest,route_id,cost FROM route INNER JOIN station ON src_station_id=station.id AND dest_station_id=station.id) as innerTable WHERE src=:start_station AND dest=:end_station")    //Alias name not working //Query 1
